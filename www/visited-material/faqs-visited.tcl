...
#EXTRA 4
set pos $Map(POS_EXTRA_4)
seek $dataFile $pos
set linea [gets $dataFile]
set patron "(BEGIN_EXTRA_4) (.*)"
...
template::multirow create Faqs ID Visits LastTime FaqUrl FaqName

while { $i > 0} {	
	set linea [gets $dataFile]
	set campos [split $linea]
	set faq_id [lindex $campos 0]
	if { [db_0or1row select_faqs {} ] } {
		multirow append Faqs $faq_id [lindex $campos 2] [user-tracking::converts_date [lindex $campos 4]] "${url}one-faq?faq_id=${faq_id}" $faq_name
	} else {
		multirow append Faqs " " [lindex $campos 2] [user-tracking::converts_date [lindex $campos 4]] "${url}one-faq?faq_id=${faq_id}" $faq_name
	}
	set i [expr $i - 1]
...
template::multirow sort Faqs -integer -decreasing Visits
