...
#EXTRA 6
set pos $Map(POS_EXTRA_6)
seek $dataFile $pos
set linea [gets $dataFile]
set patron "(BEGIN_EXTRA_6) (.*)"
...
template::multirow create News ID Visits LastTime NewsUrl Title

while { $i > 0} {	
	set linea [gets $dataFile]
	set campos [split $linea]
	set item_id [lindex $campos 0]
	if { [db_0or1row select_news {} ] } {
		multirow append News $item_id [lindex $campos 2] [user-tracking::converts_date [lindex $campos 4]] "${url}item?item_id=$item_id" $publish_title
	} else {
		multirow append News " " [lindex $campos 2] [user-tracking::converts_date [lindex $campos 4]] "${url}item?item_id=$item_id" $publish_title
	}
	set i [expr $i - 1]
...
template::multirow sort News -integer -decreasing Visits
