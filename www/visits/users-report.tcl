...
#Getting users data
set pos $Map(POS_LOGIN)
seek $dataFile $pos
set linea [gets $dataFile]
set patron "(BEGIN_LOGIN) (.*)"
...
template::multirow create Users Name ID Visits Hits LastVisit StatsLink

set url "../users-card?user_id="
...
while { $i > 0} {	
	...
	if { [db_0or1row select_user_data {} ] } {
		multirow append Users "$first_names $last_name" [lindex $campos 0] [lindex $campos 1] [lindex $campos 2] [lc_time_fmt $last_visit "%c"] "${url}[lindex $campos 0]"
	} else {
		multirow append Users " " [lindex $campos 0] [lindex $campos 1] [lindex $campos 2] [lc_time_fmt $last_visit "%c"] "${url}[lindex $campos 0]"
	}
	set i [expr $i - 1]
